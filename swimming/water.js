...
function Water() {
  ...
  this.areaConservationTexture = new GL.Texture(256, 256, { type: gl.FLOAT, filter: filter });
  ...
  if (this.textureA.type === gl.FLOAT) {
    ...
    for (let i = 0; i < this.H; i += 1) {
      for (let j = 0; j < this.W; j += 1) {
        const index = (j * this.H + i) * 4;
        const normalX = readBuf[index + 2];
        const normalZ = readBuf[index + 3];
        const normalXY_plane = new GL.Vector(normalX, Math.sqrt(1 - normalX * normalX));
        const dx = 1 / normalXY_plane.dot(new GL.Vector(0, 1)) / this.W;
        const normalZY_plane = new GL.Vector(normalZ, Math.sqrt(1 - normalZ * normalZ));
        const dz = 1 / normalZY_plane.dot(new GL.Vector(0, 1)) / this.H;


        const x = writeBuf[index];
        const z = writeBuf[index + 1];
        if (i + 1 < this.W) writeBuf[index + 4] = x + dx;
        if (j + 1 < this.H) writeBuf[index + 4 * this.W + 1] = z + dz;
      }
    }
